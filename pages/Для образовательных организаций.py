...
a = int(Raschet_ballov.at[0, 'Инорм-стенд'])
b = int(Raschet_ballov.at[0, 'Инорм-сайт']) ## количество сайт
c = a+b
e = c + 6
g = e + 5
k = g + 6
...
abz52 = otchet.add_paragraph(f"Рекомендации по исправлению недостатков.")
...
for index, row in output_df.iterrows():
    ...
    row_cells[1].text = f"Разместить на стенде организации следующие документы: {str(row['bad_stend'])}\n"\
                        ...
                        f"Обеспечить комфортность условий предоставления услуг: {str(row['bad_komf'])}\n"\
                        f"Обеспечить наличие оборудования для инвалидов: {str(row['obor_inv'])}\n"\
                        f"Обеспечить доступность среды для инвалидов: {str(row['sreda_inv'])}\n"\
