...
for index, row in output_df.iterrows():
    ...
    if Raschet_ballov.loc[(Raschet_ballov['Наименование организации или П/Н по списку'] == row['Name_org']) & (Raschet_ballov['Пдостуд'] < 90)].values[0]:
        result_k3 = "Провести инструктаж и обучение сотрудников по вопросам обеспечения доступности организации для инвалидов, в т.ч.:
- по сопровождению маломобильных граждан в помещениях организации и на прилегающей территории с учетом имеющихся у них ограничений и расстройств,
- по оказанию содействия инвалиду в беспрепятственном получении услуги.
Определить и закрепить в должностных инструкциях и распорядительных документах ответственность конкретных работников (категорий работников) организации по сопровождению маломобильных граждан и оказанию им содействия в получении услуги.
Провести оптимизацию расположения кабинетов организации с учетом обеспечения их доступности для маломобильных граждан, перенести наиболее востребованные кабинеты и услуги на первый этаж"
    else:
        result_k3 = "Получатели услуг удовлетворены доступностью услуг для инвалидов"
    
    # Добавляем результат проверки условия в столбец "Недостатки"
    if table15.loc[(table15['Наименование организации или П/Н по списку'] == row['Name_org']) & (table15['К4'] < 90)].values[0]:
        result_k4 = "1. Провести обучение персонала организации по вопросам этики и деонтологии
2. Провести обучение персонала организации по вопросам клиентоцентричности, реализации стандарта «Государство для людей» и принципов проактивного предоставления услуг
3. Ввести на регулярной основе (не реже 1 раза в квартал) рабочие совещания с коллективом организации по вопросам соблюдения норм профессиональной этики и правил служебного поведения
4. Разработка внутреннего стандарта и памятки по предоставлению информации по телефону для специалистов организации
5. Внести изменения в трудовой договор с работниками организации, включив результаты НОК (отдельные критерии/показатели) в состав показателей результативности каждого работника, осуществляющего взаимодействие с получателями услуг, для установления стимулирующих выплат и использования в рамках программ нематериальной мотивации"
    else:
        result_k4 = "нет недостатков"
    
    # Добавляем результат проверки условия в столбец "Недостатки"
    if table15.loc[(table15['Наименование организации или П/Н по списку'] == row['Name_org']) & (table15['К5'] < 90)].values[0]:
        result_k5 = "1.
Провести внутренний аудит системы менеджмента качества в структурных подразделениях организации с целью реализации превентивных мер, направленных на совершенствование условий оказания услуг по перечню недостатков, выявленных в других организациях или структурных подразделениях (управление по прецедентам)
2.
Провести анкетирование среди получателей социальных услуг, направленное на выявление глубинных причин неудовлетворенности условиями оказания услуг 3.
Разработать карты клиентского пути, направленные на уточнение и оптимизацию сценариев и условий оказания услуг в организации
4.
Разработать и провести информационную кампанию по информированию граждан о формах и видах оказываемых услуг, существующих преимуществах получения их в данной организации, возможных льготах и пр. (изготовление памяток, буклетов, написание статей в СМИ и т.п.)
5.
Внедрить в практику работы организации проекты, направленные на повышение эффективности предоставления социальных услуг и работу с целевыми категориями получателей услуг (Например, созданию условий для обучения людей с ментальными нарушениями, «Территория возможностей для граждан серебряного возраста» по организации и поддержке творческого досуга граждан пожилого возраста, удовлетворению и развитию их культурных, духовных потребностей)
6.
На уровне уполномоченного органа власти (учредителя организации) внести изменения в трудовой договор с руководителями организаций социальной сферы, включив общие результаты НОК и результаты исполнения планов по устранению недостатков, выявленных в ходе НОК)» в показатели результативности для установления стимулирующих выплат и использования в рамках программ
нематериальной мотивации"
    else:
        result_k5 = "нет недостатков"
    ...
    row_cells[3].text = f"Недостатки на стенде: {str(row['bad_stend'])}\n"\
                        ...
                        f"Результат проверки: {result_k3}\n"\
                        f"Результат проверки: {result_k4}\n"\
                        f"Результат проверки: {result_k5}"
